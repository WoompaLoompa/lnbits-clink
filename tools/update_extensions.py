"""Update `extensions.json` for the LNbits extension registry.

Mirrors `lnbits/lnbits-extensions/update_version.py` but parameterised for
extensions hosted outside the `lnbits` GitHub org (this repo lives under
`WoompaLoompa`).

Usage:
    python tools/update_extensions.py <repo_name> <version> [owner]

Example:
    python tools/update_extensions.py lnbits-clink v0.1.0
"""
...
if len(sys.argv) < 3:
    print("Usage: python tools/update_extensions.py <repo_name> <version> [owner]")
    sys.exit(1)

repo_name = sys.argv[1]
version = sys.argv[2]
owner = sys.argv[3] if len(sys.argv) > 3 else "WoompaLoompa"
...
# check if min_lnbits_version is different
    ...
    # max version is the min_lnbits_version of the current release
    extensions["extensions"][latest_index]["max_lnbits_version"] = config.get(
        "min_lnbits_version"
    )
else:
    ...
    extensions["extensions"][latest_index]["short_description"] = config.get(
        "short_description"
    )
